  ...
  if (articuls.includes(articulOnPage)) {
    ...
    let atrAnotherTry = itemData
      ...
      .getElementsByClassName(
        "card_main_content_unit_content_article"
      )[0].innerHTML;
    ...
    if (atrAnotherTry && articuls.includes(atrAnotherTry)) {
      ...
      console.log("found");
      let slider = itemData
        .getElementsByClassName("card_main_sliders")[0]
...
const parseArtelamp = async (articulsToBeFound) => {
  ...
  for (let article of articulsToBeFound) {
    ...
    let links = document
      .getElementsByClassName("listing_content")[0]
    ...
    if (links) {
      ...
      for (let i = 0; i < linksLength && !foundAll; i++) {
        let articulContainer = links[i]
          .getElementsByClassName("element")[0]
          .getElementsByClassName("the_content")[0]
          .getElementsByClassName("article")[0]
          .getElementsByTagName("a");
        let articulOnPage =
          articulContainer[articulContainer.length - 1].innerHTML;
        ...
        let relLink =
          articulContainer[articulContainer.length - 1].getAttribute("href");
